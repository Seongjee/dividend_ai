...
st.markdown(
    f"""
    #### 배당 흐름 <span style="font-size:0.9rem; color:#888;">(월 {int(st.session_state.monthly_need_m):,}만원 · 분기)</span>
    """,
    unsafe_allow_html=True
)
...
def format_year_month_from_ym(target_str):
    ...
    return f"{target_str} ({years}년)" if months == 0 else f"{target_str} ({years}년 {months}개월 후)"
...
if cross_date is not None:
    ...
    years = diff_months // 12
    months = diff_months % 12
    period_text = f"{years}년" if months == 0 else f"{years}년 {months}개월 후"
...
st.markdown(f"""
*- 배당 생활 시작(월 {int(st.session_state.monthly_need_m):,}만원) : {start_text}*  
*- 월 300만원 달성 : {target_text}*
""")
...
with t1:
    st.markdown(
        "#### 배당 시뮬레이션 <span style='font-size:0.9rem; color:#888888;'> (단위: 원)</span>", unsafe_allow_html=True)
